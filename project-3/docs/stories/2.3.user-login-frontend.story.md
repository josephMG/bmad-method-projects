...
- [ ] **AC: Login Form Display**
  - [ ] Create the login page and route (`/login`) in Next.js.
  - [ ] Develop the `LoginForm` component with fields for username/email and password.
...
- [ ] **AC: Input Validation**
  - [ ] Implement client-side validation for username/email (format) and password (not empty).
...
- [ ] **AC: Successful Login & JWT Handling**
  ...
  - [ ] Implement the logic to send login credentials to `/api/v1/token` endpoint.
  - [ ] Handle successful API response: securely store the received JWT token (e.g., in HTTP-only cookies or local storage as per `ui-spec.md`).
  - [ ] Redirect to a dashboard or profile page upon successful login.
  - [ ] Display a success message to the user upon successful login.

- [ ] **AC: Error Handling**
  - [ ] Implement error handling for backend API responses (e.g., invalid credentials, server errors).
