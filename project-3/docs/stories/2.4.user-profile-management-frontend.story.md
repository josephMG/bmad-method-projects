...
## Tasks / Subtasks
- [ ] **AC: View Profile Display**
  - [ ] Create the profile page and route (`/dashboard/settings/profile`) in Next.js.
  - [ ] Develop the `ProfileForm` component to display user details.
  - [ ] Implement logic to fetch current user details from `/api/v1/users/me` and pre-fill the form.
  - [ ] Ensure form elements adhere to WCAG 2.1 AA accessibility standards (ARIA labels, keyboard navigation).

- [ ] **AC: Update Profile Form**
  - [ ] Implement functionality to allow modification of user details (e.g., name, email) in the `ProfileForm`.
  - [ ] Add a submit mechanism (e.g., button) to trigger profile updates.

- [ ] **AC: Input Validation for Update**
  - [ ] Implement client-side validation for updated user details (e.g., email format, required fields).
...
- [ ] **AC: Successful Profile Update**
  - [ ] Integrate Axios for sending PUT/PATCH requests to the backend API.
  - [ ] Implement the logic to send updated user details to `/api/v1/users/me` endpoint.
  - [ ] Handle successful API response: display a success message.
  - [ ] Update the displayed profile information to reflect the changes.

- [ ] **AC: Error Handling for Update**
  - [ ] Implement error handling for backend API responses during profile updates (e.g., email already exists, server errors).
