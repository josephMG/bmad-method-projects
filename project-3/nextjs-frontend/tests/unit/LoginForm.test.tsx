...
describe('LoginForm', () => {
  ...
  it('calls API and redirects on successful login', async () => {
    ...
    }, { timeout: 3000 }); // Wait for the setTimeout to complete
  });
  ...
  it('handles JWT securely on successful login (e.g., via HTTP-only cookie)', async () => {
    ...
    server.use(
      http.post('http://localhost:8000/api/v1/token', () => {
        ...
        return HttpResponse.json({ access_token: 'mock_jwt_token', token_type: 'bearer' }, {
          headers: {
            'Set-Cookie': 'access_token=mock_jwt_token; HttpOnly; Path=/; Max-Age=3600',
          },
        });
      })
    );
  ...
  it('prevents XSS attacks by sanitizing user input', async () => {
    ...
    expect(screen.queryByText(/<script>alert("XSS")<\/script>/i)).not.toBeInTheDocument();
